...
#ifndef PH_GAME_THING_H
#define PH_GAME_THING_H
...
namespace Phobos
{
	...
	{		
		namespace Things
		{
			class PH_GAME_API Thing: public Node
			{
				protected:
					explicit Thing(const String_t &name, UInt32_t flags = 0);
					explicit Thing(const Char_t *name, UInt32_t flags = 0);
	...
	#if 0
			typedef void (Object::*InputProc_t)(EntityEvent &);

			//Connects the given input to a signal
			typedef void (Object::*OutputProcConnector_t)(Object &ent, InputProc_t input);
