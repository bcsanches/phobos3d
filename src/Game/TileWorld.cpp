...
#include "Phobos/Game/TileTransform.h"

namespace Phobos
{	
	namespace Game
	{
		static inline bool CheckNorthTileEqual(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col, char tile)
		{
			return (row > 0) && handle(row-1, col) == tile;
		}

		static inline bool CheckWestTileEqual(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col, char tile)
		{
			return (col > 0) && handle(row, col-1) == tile;
		}

		static inline bool CheckSouthTileEqual(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col, char tile)
		{
			return (row < handle.GetNumRows() - 1) && handle(row+1, col) == tile;
		}		

		static inline bool CheckEastTileEqual(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col, char tile)
		{
			return (col < handle.GetNumColumns()-1) && handle(row, col+1) == tile;
		}

		static inline bool HasNorthWall(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckNorthTileEqual(handle, row, col, '#');
		}

		static inline bool HasWestWall(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckWestTileEqual(handle, row, col, '#');
		}

		static inline bool HasSouthWall(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckSouthTileEqual(handle, row, col, '#');
		}	

		static inline bool HasEastWall(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckEastTileEqual(handle, row, col, '#');
		}

		static inline bool IsNorthFloor(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckNorthTileEqual(handle, row, col, '.');
		}

		static inline bool IsWestFloor(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckWestTileEqual(handle, row, col, '.');
		}

		static inline bool IsSouthFloor(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckSouthTileEqual(handle, row, col, '.');
		}	

		static inline bool IsEastFloor(const Phobos::Register::Table::MatrixDataHandle &handle, int row, int col)
		{
			return CheckEastTileEqual(handle, row, col, '.');
		}
		...
		void TileWorld::CreateStaticObjectRigidBody(StaticObject_s &staticObj, const Ogre::Vector3 &scale, const Physics::CollisionTag &collisionTag)
		{
			...
			BaseOgreWorld::CreateStaticObjectRigidBody(staticObj, transform, scale, collisionTag);		
		}

		BaseOgreWorld::StaticObject_s &TileWorld::CommitTempObject(TempStaticObject_s &obj)
		{
		...
		void TileWorld::CreateStaticObjectNode(TempStaticObject_s &obj, const TileTransform &tileTransform, const Ogre::Vector3 &scale)
		{
		...
		void TileWorld::CreateStaticObjectNode(TempStaticObject_s &obj, const Phobos::Register::Table &table, const Ogre::Vector3 &scale)
		{		
			this->CreateStaticObjectNode(obj, this->CreateTileTransform(table), scale);		
		}

		void TileWorld::CreateStaticObjectMesh(TempStaticObject_s &obj, const String_t &meshName, const String_t *optionalMaterial) const
		{
		...
		void TileWorld::SpawnMesh(int row, int col, const String_t &meshName, Float_t tileScale, const Engine::Math::Transform &transform, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
		...
		void TileWorld::SpawnMesh(const TileTransform tileTransform, const String_t &meshName, const Ogre::Vector3 &scale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
			TempStaticObject_s obj;							
		...
		void TileWorld::CreateCeilingMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{		
		...
		void TileWorld::CreateFloorMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
			this->SpawnMesh(row, col, meshName, tileScale, Engine::Math::Transform(), optionalMaterial, collisionTag);						
		}

		void TileWorld::CreateNorthWallMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
		...
		void TileWorld::CreateSouthWallMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
			this->SpawnMesh(
				row, 
				col, 
		...
		void TileWorld::CreateWestWallMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
			this->SpawnMesh(
				row, 
				col, 
		...
		void TileWorld::CreateEastWallMesh(int row, int col, const String_t &meshName, Float_t tileScale, const String_t *optionalMaterial, const Physics::CollisionTag &collisionTag)
		{
			this->SpawnMesh(
				row, 
				col, 
		...
		void TileWorld::Load(const MapLoader &loader, const Phobos::Register::Table &worldEntityTable)
		{				
			auto &tileSetDef = Phobos::Register::GetHive("TileSet").GetTable(worldEntityTable.GetString("tileSet"));
			...
			Phobos::Register::Table::MatrixDataHandle handle = worldEntityTable.GetMatrix("map");
			...
			{
				auto *dict = static_cast<Phobos::Register::Table *>(it->second);
				...
				{				
					...
					if(type.compare("Light") == 0)
					{
						...
						{
							LogMakeStream() << "[TileWorld::Load] Unknown light object type: " << lightType;
						}
					}
					...
					{
						LogMakeStream() << "[TileWorld::Load] Unknown static object type: " << type;
					}
				}
				catch(Exception &ex)
				{
		...
		Ogre::Vector3 TileWorld::CalculatePosition(int row, int col) const
		{
			return Ogre::Vector3((m_fpTileSize * col) + m_fpTileSize / 2, 0, (m_fpTileSize * row) + m_fpTileSize / 2);
		}

		void TileWorld::TileTransform2Transform(Engine::Math::Transform &out, const TileTransform &tileTransform) const
		{				
		...
		void TileWorld::LoadTileTransform(Engine::Math::Transform &out, const Phobos::Register::Table &entity) const
		{
			this->TileTransform2Transform(out, this->CreateTileTransform(entity));
		}

		TileTransform  TileWorld::CreateTileTransform(const Phobos::Register::Table &entity) const
		{
			...
			if(!clDirectionType_gl.TryGetValue(dir, direction))
			{
				...
				PH_RAISE(INVALID_PARAMETER_EXCEPTION, "TileWorld::LoadTileTransform", stream.str());
			}
